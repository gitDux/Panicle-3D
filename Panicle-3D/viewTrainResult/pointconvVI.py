...
filename = 'pointconv-master\\viewTrainResult\\his\\log_train_100.txt'
...
x = np.linspace(1,89,89,endpoint=True)

l1, = plt.plot(x,test_loss[1:90],'r-')
l2, = plt.plot(x,train_loss[1:90],'b--')
plt.legend([l1, l2], ['test loss', 'train loss'], loc = 'upper right')
plt.show()

l1, = plt.plot(x,test_IoU[1:90],'r-')
l2, = plt.plot(x,train_IoU[1:90],'b--')
plt.legend([l1, l2], ['test IoU', 'train IoU'], loc = 'upper right')
plt.show()

l1, = plt.plot(x,test_acc[1:90],'r-')
