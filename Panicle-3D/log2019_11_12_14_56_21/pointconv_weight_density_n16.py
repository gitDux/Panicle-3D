...
import numpy as np
import tf_util
from PointConv import feature_encoding_layer, feature_decoding_layer, se_inception_layer
...
def get_model(point_cloud, is_training, num_class, sigma, bn_decay=None, weight_decay = None):
    ...
    # Feature encoding layers，l1_xyz, l1_points点，特征。第一层吧点坐标作为特征
    l1_xyz, l1_points = se_inception_layer(l0_xyz, l0_points, npoint=1024, radius = 0.1, sigma = sigma, K1 = 16, K2=32, mlp=[32,32,64], is_training=is_training, bn_decay=bn_decay, weight_decay = weight_decay, scope='layer1')
    l2_xyz, l2_points = se_inception_layer(l1_xyz, l1_points, npoint=256, radius = 0.2, sigma = 2 * sigma, K1 = 16, K2=32, mlp=[64,64,128], is_training=is_training, bn_decay=bn_decay, weight_decay = weight_decay, scope='layer2')
    l3_xyz, l3_points = se_inception_layer(l2_xyz, l2_points, npoint=64, radius = 0.4, sigma = 4 * sigma, K1 = 16, K2=32, mlp=[128,128,256], is_training=is_training, bn_decay=bn_decay, weight_decay = weight_decay, scope='layer3')
    l4_xyz, l4_points = se_inception_layer(l3_xyz, l3_points, npoint=36, radius = 0.8, sigma = 8 * sigma, K1 = 16, K2=32, mlp=[256,256,512], is_training=is_training, bn_decay=bn_decay, weight_decay = weight_decay, scope='layer4')
...
def get_loss(pred, label, smpw):
    """ pred: BxNxC,
        label: BxN,
	smpw: BxN """
    #classify_loss = tf.losses.sparse_softmax_cross_entropy(labels=label, logits=pred, weights=smpw)#label非onehot，weights是给每个标签的损失给一个权重
    classify_loss = tf.losses.sparse_softmax_cross_entropy(labels=label, logits=pred)
    #classify_loss = tf.losses.softmax_cross_entropy_with_logits(labels=label, logits=pred, weights=smpw)#如果label已经是onehot
