...
def nonlinear_transform(data_in, mlp, scope, is_training, bn_decay=None, weight_decay = None, activation_fn = tf.nn.relu):
    ...
    return net
#(l0_xyz, l0_points, npoint=1024, radius = 0.1, sigma = sigma, K=32, mlp=[32,32,64], is_training=is_training, bn_decay=bn_decay, weight_decay = weight_decay, scope='layer1')
def feature_encoding_layer(xyz, feature, npoint, radius, sigma, K, mlp, is_training, bn_decay, weight_decay, scope, bn=True, use_xyz=True):
    ...
    '''
    with tf.variable_scope(scope) as sc:#生成一个上下文管理器,类似命名空间
        num_points = xyz.get_shape()[1]
        if num_points == npoint:
            new_xyz = xyz
        else:
            new_xyz = pointconv_util.sampling(npoint, xyz)#最远点采样

        grouped_xyz, grouped_feature, idx = pointconv_util.grouping(feature, K, xyz, new_xyz)#在原点云中找采样后的最近邻

        density = pointconv_util.kernel_density_estimation_ball(xyz, radius, sigma)#计算密度
        ...
        inverse_max_density = tf.reduce_max(grouped_density, axis = 2, keepdims = True)#最大密度
        density_scale = tf.div(grouped_density, inverse_max_density)#密度归一化

        #density_scale = tf_grouping.group_point(density, idx)

        for i, num_out_channel in enumerate(mlp):
            if i != len(mlp) - 1:
                grouped_feature = tf_util.conv2d(grouped_feature, num_out_channel, [1,1],#二维卷积代替mlp
                                            padding='VALID', stride=[1,1],
        ...
        return new_xyz, new_points#返回采样后的点和特征

def senet_layer(xyz, feature, npoint, radius, sigma, K, mlp, is_training, bn_decay, weight_decay, scope, bn=True, use_xyz=True,reduction_ratio=4):
    new_xyz, new_feature = feature_encoding_layer(xyz, feature, npoint, radius, sigma, K, mlp, is_training, bn_decay, weight_decay, scope)
    squeeze = tf.reduce_mean(new_feature, [1], name='se_pool', keep_dims=True)
    excitation = tf.layers.dense(inputs=squeeze, units=int(mlp[-1]/reduction_ratio), activation=tf.nn.relu)
    excitation = tf.layers.dense(inputs=excitation, units=mlp[-1], activation=tf.nn.sigmoid)
    out = new_feature*excitation
    return new_xyz, out

def inception_layer(xyz, feature, npoint, radius, sigma, K1, K2, mlp, is_training, bn_decay, weight_decay, scope, bn=True, use_xyz=True):
    div_mlp = [int(i/2) for i in mlp]
    xyz1, feature1 = feature_encoding_layer(xyz, feature, npoint, radius, sigma, K1, div_mlp, is_training, bn_decay, weight_decay, scope+'_1')
    xyz2, feature2 = feature_encoding_layer(xyz, feature, npoint, radius, sigma, K2, div_mlp, is_training, bn_decay, weight_decay, scope+'_2')
    #xyz1 = xyz2
    new_feature = tf.concat(axis=-1, values=[feature1, feature2]) 
    return xyz1, new_feature

def se_inception_layer(xyz, feature, npoint, radius, sigma, K1, K2, mlp, is_training, bn_decay, weight_decay, scope, bn=True, use_xyz=True,reduction_ratio=4):
    new_xyz, new_feature = inception_layer(xyz, feature, npoint, radius, sigma, K1, K2, mlp, is_training, bn_decay, weight_decay, scope+'inception')
    squeeze = tf.reduce_mean(new_feature, [1], name=scope+'se_pool', keep_dims=True)
    excitation = tf.layers.dense(inputs=squeeze, units=int(mlp[-1]/reduction_ratio), activation=tf.nn.relu)
    excitation = tf.layers.dense(inputs=excitation, units=mlp[-1], activation=tf.nn.sigmoid)
    out = new_feature*excitation
    return new_xyz, out
...
def feature_decoding_layer(xyz1, xyz2, points1, points2, radius, sigma, K, mlp, is_training, bn_decay, weight_decay, scope, bn=True, use_xyz = True):
    ...
    with tf.variable_scope(scope) as sc:
        ...
        norm = tf.tile(norm,[1,1,3])#3个维度分别复制1，1，3次
        weight = (1.0/dist) / norm
        interpolated_points = three_interpolate(points2, idx, weight)#对特征进行内插，实现xyz2的特征->xyz1特征，后面再卷积修正

        #setup for deConv，高维上的k近邻
